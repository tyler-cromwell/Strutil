...
char* strutil_removeThese(char* string, char* chars) {
#ifdef STRUTIL_DEBUG
    fprintf(stdout, COLOR_YELLOW "STRUTIL: REMOVE_THESE: Attempting to remove characters \"%s\" from \"%s\"\n" COLOR_RESET, chars, string);
#endif
    ...
    for (size_t i = 0; i < strlen(string); i++) {
        for (size_t j = 0; j < strlen(chars); j++) {
            if (string[i] != chars[j]) {
#ifdef STRUTIL_DEBUG
                fprintf(stdout, COLOR_YELLOW "STRUTIL: REMOVE_THESE: \'%c\'\n" COLOR_RESET, string[i]);
#endif
...
#ifdef STRUTIL_DEBUG
    fprintf(stdout, COLOR_YELLOW "STRUTIL: REMOVE_THESE: \"%s\" shortened to \"%s\"\n" COLOR_RESET, string, shortened);
#endif
    return shortened;
}
