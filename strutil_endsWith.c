...
#ifdef STRUTIL_DEBUG
        fprintf(stdout, COLOR_YELLOW "STRUTIL: ENDS_WITH: Token \"%s\" is longer than \"%s\".\n" COLOR_RESET, endtok, string);
#endif
        return 0;
    }

#ifdef STRUTIL_DEBUG
    fprintf(stdout, COLOR_YELLOW "STRUTIL: ENDS_WITH: Comparing the last \'%zu\' byte(s) of \"%s\" to \"%s\"\n" COLOR_RESET, strlen(endtok), string, endtok);
#endif
...
#ifdef STRUTIL_DEBUG
        fprintf(stdout, COLOR_YELLOW "STRUTIL: ENDS_WIDTH: \"%s\" ends with \"%s\".\n" COLOR_RESET, string, endtok);
#endif
        free(substr);
        return 1;
    } else {
#ifdef STRUTIL_DEBUG
        fprintf(stdout, COLOR_YELLOW "STRUTIL: ENDS_WIDTH: \"%s\" does not end with \"%s\".\n" COLOR_RESET, string, endtok);
#endif
        free(substr);
        return 0;
    }
